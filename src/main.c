...
static void night_set_night_routes (HttpCerver *http_cerver) {

	/* register top level route */
	// GET /night
	HttpRoute *main_route = http_route_create (REQUEST_METHOD_GET, "night", night_handler);
	http_cerver_route_register (http_cerver, main_route);

	/* register night children routes */
	// GET night/version
	HttpRoute *version_route = http_route_create (REQUEST_METHOD_GET, "version", night_version_handler);
...
static void start (void) {
	...
	if (test_api) {
		...
		night_set_night_routes (http_cerver);

		// add a catch all route
		http_cerver_set_catch_all_route (http_cerver, night_catch_all_handler);

		// admin configuration
		http_cerver_enable_admin_routes (http_cerver, true);
		http_cerver_register_admin_file_system (http_cerver, "/var/uploads");
		http_cerver_register_admin_file_system (http_cerver, "/home/night/uploads");
...
int main (int argc, char const **argv) {
	...
	if (!night_init ()) {
		...
		cerver_log_error ("Failed to init night!");
	}
